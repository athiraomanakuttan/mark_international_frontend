...
import { trannsferLead } from "@/service/admin/leadService"


interface TransferLeadProps {
    leadList: string[],
  open: boolean
...
export default function TransferLeadModal({ open, setOpen, leadList, transferSuccess }: TransferLeadProps) {
    const dispatch = useDispatch<AppDispatch>();
      ...
      const handleSubmit = async (e:React.FormEvent)=>{
        ...
        try {
            const response = await trannsferLead(selectedStaff, leadList)
            if(response.status){
              toast.success("Lead transfer successfull")
              setOpen(false)
              transferSuccess()
            }

        } catch (error) {
          toast.error("unable to transfer the lead. Try again")
        }
      }
